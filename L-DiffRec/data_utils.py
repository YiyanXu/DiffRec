...
class SubData(Dataset):
    def __init__(self, train_path, valid_path, test_path, num_sub=1000):
        ...
        self.train_dict, self.valid_dict, self.test_dict, \
            self.num_user, self.num_item = subdata_load(
    ...
    def get_val(self, data):
        ...
        for uid in data:
            ...
            try:
                ...
                print("Error!")
            m = np.random.choice(
                np.array(list(a)), self.num_sub-len(val_list[uid]), replace=False)
    ...
    def get_test(self, data):
        ...
        for uid in data:
            ...
            gt_list[uid].extend([i for i in range(len(data[uid]))])
            try:
                m = np.random.choice(np.array(list(self.item_set-set(self.train_dict[uid])-set(
                    self.valid_dict[uid]))), self.num_sub-len(test_list[uid]), replace=False)
            except:
                m = np.random.choice(np.array(list(
                    self.item_set-set(self.train_dict[uid]))), self.num_sub-len(test_list[uid]), replace=False)
